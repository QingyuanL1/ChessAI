...
from sources.chess.lookup_tables import ActionLabelsRed, flip_policy, flip_move
...
class SelfPlayWorker:
    ...
    def start(self):
        self.pid = os.getpid()
        logger.debug(f"#Start Process index = {self.id}, pid = {self.pid}")
    ...
    def start_game(self, idx, search_tree):
        ...
        self.player = AI_Player(self.config, search_tree=search_tree, pipes=pipes, enable_resign=enable_resign, debugging=False)

        state = senv.INIT_STATE
        history = [state]
        value = 0
        turns = 0       # 偶数red 奇数black
        game_over = False
    ...
    def get_ucci_move(self, fen, time=3):
        ...
        setrandom = f'setoption randomness {self.config.trainsetting.random}\n'
