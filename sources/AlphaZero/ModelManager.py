...
class ModelManager:
    ...
    def build(self):
        ...
        # In TensorFlow 2.x, we don't need to explicitly get the default graph
    ...
    def get_pipes(self, num=1, api=None, need_reload=True):
        if self.Predictor is None:
            self.Predictor = Predictor(self.config, self)
            self.Predictor.start(need_reload)
        return self.Predictor.get_pipe(need_reload)
