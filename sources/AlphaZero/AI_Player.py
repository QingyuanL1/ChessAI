...
class AI_Player:
    def __init__(self, config: Config, search_tree=None, pipes=None, play_config=None, 
        ...
        self.pipe = pipes                   # pipes that used to communicate with Module thread
    ...
    def apply_temperature(self, policy, turn) -> np.ndarray:
        if turn < 30 and self.play_config.tau_decay_rate != 0:
            ...
            tau = 0
        if tau < 0.1 or (turn >= 4 and self.config.trainsetting.evaluate):
             tau = 0
        if self.increase_temp and not self.config.trainsetting.evaluate:
            tau = 0.5
        if tau == 0:
